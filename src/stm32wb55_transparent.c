...
typedef struct {
    mp_obj_t rfcore_ble_hci;  // stm.rfcore_ble_hci function
...
STATIC mp_obj_t rfcore_transparent(mp_obj_t stream_in, mp_obj_t stream_out, mp_obj_t callback) {
    ...
    if (!trans_state.initialized) {
        // Import stm module and get rfcore_ble_hci function
        mp_obj_t stm_module = mp_import_name(MP_QSTR_stm, mp_const_none, MP_OBJ_NEW_SMALL_INT(0));
        trans_state.rfcore_ble_hci = mp_import_from(stm_module, MP_QSTR_rfcore_ble_hci);
    ...
    if (trans_state.state == STATE_IN_PAYLOAD && trans_state.len == 0) {
        ...
        if (trans_state.cmd_type == HCI_KIND_LOCAL_CMD) {
            ...
            // Forward raw HCI packet to Bluetooth controller via stm.rfcore_ble_hci()
            DEBUG_printf("rfcore_ble_hci len=%d\n", trans_state.rx);

            // Create bytes object for the complete raw HCI packet
            mp_obj_t cmd_bytes = mp_obj_new_bytes(trans_state.buf, trans_state.rx);

            // Create a mutable bytearray for the response
            mp_obj_t rsp_bytearray = mp_obj_new_bytearray_by_ref(sizeof(trans_state.rsp_buf), trans_state.rsp_buf);

            // Call stm.rfcore_ble_hci(cmd, rsp) which returns response length
            mp_obj_t args[] = {cmd_bytes, rsp_bytearray};
            mp_obj_t result = mp_call_function_n_kw(trans_state.rfcore_ble_hci, 2, 0, args);
            rsp_len = mp_obj_get_int(result);

            // Copy response to buf for writing
            if (rsp_len > 0 && rsp_len <= sizeof(trans_state.buf)) {
                mp_fun_table.memmove_(trans_state.buf, trans_state.rsp_buf, rsp_len);
            }
        }
